...
if __name__ == "__main__":
  ...
  - 'hallo3_keepdim': Hallo3 but keeps audio_dim=384 (LatentSync-consistent)""",
    ...
    # Curated guardrails for the simplified script.
    # This keeps only the requested feature paths active.
    disallowed = []
    if not getattr(args, "use_latentsync_audio", False):
        disallowed.append("require --use_latentsync_audio")
    if not getattr(args, "extract_audio_embeddings_online", False):
        disallowed.append("require --extract_audio_embeddings_online")
    if not getattr(args, "use_causal_wan", False):
        disallowed.append("require --use_causal_wan")
    if getattr(args, "use_stableavatar", False):
        disallowed.append("--use_stableavatar not supported in train_simple.py")
    if getattr(args, "use_stableavatar_audio", False):
        disallowed.append("--use_stableavatar_audio not supported in train_simple.py")
    if getattr(args, "use_precomputed_context", False):
        disallowed.append("--use_precomputed_context not supported in train_simple.py")
    if getattr(args, "use_precomputed_latents", False):
        disallowed.append("--use_precomputed_latents not supported in train_simple.py")
    if disallowed:
        raise ValueError("train_simple.py guardrails: " + "; ".join(disallowed))
        ...
        if val_mode is not None and val_dataset is not None and len(val_dataset) > 0 and accelerator.is_main_process:
            ...
            def preprocess_with_latentsync(
                ...
                try:
                    ...
                    audio_emb = None
                    if extract_audio and wav2vec_feature_extractor is not None and wav2vec_model is not None:
                        # try:
                        ...
                        # except Exception as e:
                        #     print(f"[Wav2Vec] ERROR: Audio extraction failed for {video_id}: {e}")
                        #     audio_emb = None
