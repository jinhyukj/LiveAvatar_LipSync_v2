...
from diffsynth.models.syncnet.detectors import S3FD


class SyncNetDetector:
    def __init__(self, device, detect_results_dir="detect_results", debug_crop_dir=None, s3fd_model_path=None):
        self.s3f_detector = S3FD(device=device, model_path=s3fd_model_path)
        self.detect_results_dir = detect_results_dir
        self.debug_crop_dir = debug_crop_dir or None
    ...
    def __call__(self, video_path: str, min_track=50, scale=False):
        ...
        if self.debug_crop_dir:
            ...
            debug_root = os.path.join(self.debug_crop_dir, video_stem)
        ...
        for ii, track in enumerate(alltracks):
            self.crop_video(
                ...
                temp_dir,
                video_dir,
                debug_dir=os.path.join(debug_root, f"{ii:05d}") if debug_root else None,
            )
    ...
    def crop_video(
        ...
        for fidx, frame in enumerate(track["frame"]):
            ...
            if debug_dir:
                cv2.imwrite(os.path.join(debug_dir, f"{fidx:05d}.png"), face_resized)
                if fidx == 0:
                    print(f"[SyncNet][debug] {debug_dir}: frame shape {face_resized.shape}")
