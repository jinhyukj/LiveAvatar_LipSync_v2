...
from diffsynth.models.syncnet.syncnet.syncnet import S
from shutil import rmtree
from diffsynth.models.syncnet.utils import check_model_and_download
...
class SyncNetEval(torch.nn.Module):
    ...
    def evaluate(self, video_path, temp_dir="temp", batch_size=20, vshift=15):      ## video_path = 'detect_results/crop/00000.mp4'
        ...
        command = f"ffmpeg -loglevel error -nostdin -y -i {video_path} -f image2 {os.path.join(temp_dir, '%06d.jpg')}"  ## temp 파일 만들고 거기 안에 video의 frame들을 000001.jpg, 000002.jpg,... 형태로 저장
        subprocess.call(command, shell=True, stdout=None)

        command = f"ffmpeg -loglevel error -nostdin -y -i {video_path} -async 1 -ac 1 -vn -acodec pcm_s16le -ar 16000 {os.path.join(temp_dir, 'audio.wav')}" # temp/audio.wav 파일 생성
        subprocess.call(command, shell=True, stdout=None)
        ...
        for fname in flist:
            ...
            img_input = cv2.resize(img_input, (224, 224))  # HARD CODED, CHANGE BEFORE RELEASE      ## resizes to 224x224
        ...
        imtv = torch.autograd.Variable(torch.from_numpy(im.astype(float)).float())          ## torch.Size([1, 3, 139, 224, 224]) [batch, channel, frame, height, width]
        ...
        cct = torch.autograd.Variable(torch.from_numpy(cc.astype(float)).float())       ## torch.Size([1, 1, 13,  549])  [batch, channel, feature_dim, time_steps]
        ...
        min_length = min(len(images), math.floor(len(audio) / 640))  ## finds the minimum length in terms of number of video frames
        ...
        tS = time.time()
        for i in range(0, lastframe, batch_size):       ## processes 20 frames(batch_size) at once

            im_batch = [imtv[:, :, vframe : vframe + 5, :, :] for vframe in range(i, min(lastframe, i + batch_size))]    ## list of tensors, each tensor shape: torch.Size([1, 3, 5, 224, 224]), 20 tensors if batch_size=20   
            im_in = torch.cat(im_batch, 0)      ## torch.Size([20, 3, 5, 224, 224])
