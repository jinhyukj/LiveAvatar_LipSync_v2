...
class WanS2VSelfAttention(WanSelfAttention):
    ...
    def forward(self, x, seq_lens, grid_sizes, freqs,sp_size=None):
        """
        Args:
...
class WanModel_S2V(ModelMixin, ConfigMixin):
    ...
    def after_transformer_block(self, block_idx, hidden_states):
        if block_idx in self.audio_injector.injected_block_id.keys():
            ...
            if self.use_context_parallel:
                hidden_states = gather_forward(hidden_states, dim=1, sp_size=self.sp_size)
            ...
            if self.use_context_parallel:
                hidden_states, _ = pad_chunk(hidden_states, self.sp_size, dim=1)
    ...
    def forward(
        ...
        audio_emb_res = self.casual_audio_encoder(audio_input)#->float32
        if self.enbale_adain:
            audio_emb_global, audio_emb = audio_emb_res
            self.audio_emb_global = audio_emb_global[:,
                                                     motion_frames[1]:].clone().to(dtype=torch.bfloat16)
        else:
            audio_emb = audio_emb_res
        self.merged_audio_emb = audio_emb[:, motion_frames[1]:, :].to(dtype=torch.bfloat16)
        ...
        if self.zero_timestep:
            e = e[:-1] # [b,dim]
            zero_e0 = e0[-1:] # [1,6,dim]
            e0 = e0[:-1] # [b,6,dim]
            token_len = x.shape[1]
            e0 = torch.cat([
                e0.unsqueeze(2), # [b,6,1,dim]
                zero_e0.unsqueeze(2).repeat(e0.size(0), 1, 1, 1) # [b,6,1,dim]
            ], 
                           dim=2) # [b,6,2,dim]
            e0 = [e0, self.original_seq_len]
        else:
            e0 = e0.unsqueeze(2).repeat(1, 1, 2, 1) # [b,6,2,dim]
            e0 = [e0, 0]
        ...
        # Context Parallel
        if self.use_context_parallel:
            x = gather_forward(x.contiguous(), dim=1, sp_size=self.sp_size)
        # unpatchify
